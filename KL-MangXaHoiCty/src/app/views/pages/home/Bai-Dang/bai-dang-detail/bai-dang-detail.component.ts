...
import { BaidangEditComponent } from '../baidang-edit/baidang-edit.component';
import { TinNhanhEditComponent } from '../tin-nhanh-edit/tin-nhanh-edit.component';
import { DeXuatEditComponent } from '../de-xuat-edit/de-xuat-edit.component';
import { ChaoDonThanhvienEditComponent } from '../chao-don-thanhvien-edit/chao-don-thanhvien-edit.component';
import { KhenThuongEditComponent } from '../khen-thuong-edit/khen-thuong-edit.component';
...
@Component({
  selector: 'kt-bai-dang-detail',
  templateUrl: './bai-dang-detail.component.html',
  styleUrls: ['./bai-dang-detail.component.scss']
})
export class BaiDangDetailComponent implements OnInit {
  item_11:any[]=[];
  data: any[] = [];
  listTT_user:any[]=[];
 
  @Input() id_baidang: any;
  id_user:number;
  list_detail:any[]=[];
...
// 	this._signalRService.cmt.subscribe((announcement: any) => {
// 	  this._ngZone.run(() => {
...
// 			console.log('List trả lời cmt:',this.data );
// 			// this.loadDataList();
// 			this.changeDetectorRefs.detectChanges();
...
Item_thongbao_chikd(): ThongBaoModel {
    const item = new ThongBaoModel();
  

		 item.title="Đã trả lời bình luận của bạn";
		 item.id_cmt=1;
  ...
  AddThongbao_child(item:ThongBaoModel,withBack:boolean){
      ...
      this._service_thongbao.InsertThongBao(item).subscribe(res=>{
        ...
        ThongBaotInsert_child()
        {
  
          let it_cmt=this.Item_thongbao_chikd();
           this.AddThongbao_child(it_cmt,false);
...
// load  dữ liệu bài đăng (cmt,like) bằng datasource để realtime
  loadDataList() {
	...
	this._services.getDetailBaiDang(this.id_user,this.id_baidang).subscribe((res) => {
